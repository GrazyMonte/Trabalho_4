#define _CRT_SECURE_NO_WARNINGS
#include <stdio.h>
#include <stdlib.h>
#include <iostream>

int main()
{
	FILE *fp = NULL, *arquivo_aux = NULL;
	FILE *fp_1 = NULL, *arquivo_aux_1 = NULL;
	char id;
	int a, larg, alt, maior, aux = 0, x, y, i, j;

	int **matriz = NULL, **m_limiar = NULL;
	int menor, limiar;
	long int cont = 0, cont_aux = 0;

	fp = fopen("D:/Backup Zizi/Disco Local C/Documentos/Programacao_Avancada/Trabalho_4_/barbara.ascii.pgm", "r");

	fscanf(fp, "%c%d %d %d %d", &id, &a, &larg, &alt, &maior);
	printf("%c%d %d %d %d \n", id, a, larg, alt, maior);

	matriz = new int*[alt];
	for (i = 0; i < alt; i++) matriz[i] = new int[larg];

	for (i = 0; i < alt; i++)
	{
		for (j = 0; j < larg; j++)
		{
			fscanf(fp, "%d", &aux);
			matriz[i][j] = aux;
			cont++;
		}
	}

	menor = matriz[0][0];
	for (int i = 0; i < alt; i++)
	{
		for (int j = 0; j < larg; j++)
		{
			if (matriz[i][j] < menor) menor = matriz[i][j];
		}
	}

	limiar = (maior - menor) / 2;
	printf("maior = %d \nmenor = %d \nlimiar = %d", maior, menor, limiar);

	m_limiar = new int*[alt];
	for (i = 0; i < alt; i++) m_limiar[i] = new int[larg];

	for (int i = 0; i < alt; i++)
	{
		for (int j = 0; j < larg; j++)
		{
			if (matriz[i][j] > limiar) m_limiar[i][j] = 255;
			else m_limiar[i][j] = 0;
		}
	}

	arquivo_aux = fopen("D:/Backup Zizi/Disco Local C/Documentos/Programacao_Avancada/Trabalho_4_/Limiar/Limiar_barbara.ascii.pgm", "w");

	fprintf(arquivo_aux, "%c%d %d %d 255 \n", id, a, larg, alt);

	for (int i = 0; i < alt; i++)
	{
		for (int j = 0; j < larg; j++)
		{
			aux = m_limiar[i][j];
			fprintf(arquivo_aux, "%d ", aux);
			cont_aux++;
		}
	}

	fclose(arquivo_aux);

	aux = 0;
	cont = 0;
	cont_aux = 0;

	fp_1 = fopen("D:/Backup Zizi/Disco Local C/Documentos/Programacao_Avancada/Trabalho_4_/mother_daughter.ascii.pgm", "r");

	fscanf(fp_1, "%c%d %d %d %d", &id, &a, &larg, &alt, &maior);
	printf("\n\n%c%d %d %d %d \n", id, a, larg, alt, maior);

	matriz = new int*[alt];
	for (i = 0; i < alt; i++) matriz[i] = new int[larg];

	for (i = 0; i < alt; i++)
	{
		for (j = 0; j < larg; j++)
		{
			fscanf(fp_1, "%d", &aux);
			matriz[i][j] = aux;
			cont++;
		}
	}

	menor = matriz[0][0];
	for (int i = 0; i < alt; i++)
	{
		for (int j = 0; j < larg; j++)
		{
			if (matriz[i][j] < menor) menor = matriz[i][j];
		}
	}

	limiar = (maior - menor) / 2;
	printf("maior = %d \nmenor = %d \nlimiar = %d", maior, menor, limiar);

	m_limiar = new int*[alt];
	for (i = 0; i < alt; i++) m_limiar[i] = new int[larg];

	for (int i = 0; i < alt; i++)
	{
		for (int j = 0; j < larg; j++)
		{
			if (matriz[i][j] > limiar) m_limiar[i][j] = 255;
			else m_limiar[i][j] = 0;
		}
	}

	arquivo_aux_1 = fopen("D:/Backup Zizi/Disco Local C/Documentos/Programacao_Avancada/Trabalho_4_/Limiar/Limiar_mother_daughter.ascii.pgm", "w");

	fprintf(arquivo_aux_1, "%c%d %d %d 255 \n", id, a, larg, alt);

	for (int i = 0; i < alt; i++)
	{
		for (int j = 0; j < larg; j++)
		{
			aux = m_limiar[i][j];
			fprintf(arquivo_aux_1, "%d ", aux);
			cont_aux++;
		}
	}

	fclose(arquivo_aux_1);

	for (i = 0; i < alt; i++) delete[] m_limiar[i];
	delete[] m_limiar;

	for (i = 0; i < alt; i++) delete[] matriz[i];
	delete[] matriz;

	fclose(fp_1);

	system("pause");
	return 0;

}